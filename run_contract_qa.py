"""Run BERT on Elements Contract Extraction for Japanese"""
...
flags.DEFINE_string("train_file", None,
                    "Contract json for training. E.g., train-v1.1.json")

flags.DEFINE_string(
    "predict_file", None,
    "Contract json for predictions. E.g., dev-v1.1.json or test-v1.1.json")
...
flags.DEFINE_bool(
    ...
    "A number of warnings are expected for a normal Contract evaluation.")
flags.DEFINE_bool(
    "version_2_with_negative", False,
    "If true, the Contract examples contain some that do not have an answer.")
flags.DEFINE_float(
...
class _LogSessionRunHook(tf.train.SessionRunHook):
    ...
    def after_run(self, run_context, run_values):
        ...
        if print_step == 1 or print_step % self.display_every == 0:
            ...
            print('%6i %11.1f %6.6f     %6.6e' %
                  (print_step, img_per_sec, total_loss, lr))
            self.elapsed_secs = 0.
            self.count = 0


class ContractExample(object):
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        query_tokens = [tok.lower() for tok in example.question_text]
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
                      ...
                      output_nbest_file, output_null_log_odds_file, is_writed=True):
    ...
    _PrelimPrediction = collections.namedtuple(  # pylint: disable=invalid-name
        "PrelimPrediction",
        ["feature_index", "start_index", "end_index", "start_logit", "end_logit", "question_length"])
                ...
                for end_index in end_indexes:
                    ...
                    prelim_predictions.append(
                        _PrelimPrediction(
                            ...
                            question_length=len(example.question_text)))
        ...
        for pred in prelim_predictions:
            ...
            nbest.append(
                _NbestPrediction(
                    ...
                    start_index=pred.start_index - pred.question_length - 2 + pred.feature_index * FLAGS.doc_stride,
                    end_index=pred.end_index - pred.question_length - 2 + pred.feature_index * FLAGS.doc_stride))
            ...
            if "" not in seen_predictions:
                nbest.append(
                    _NbestPrediction(
                        ...
                        end_logit=null_end_logit,
                        start_index=null_start_logit,
                        end_index=null_end_logit))
...
def get_final_text(pred_text, orig_text, do_lower_case):
    ...
    # (the Contract eval script also does punctuation stripping/lower casing but
...
def main(_):
    ...
    if FLAGS.do_train:
        train_examples = read_contract_examples(input_file=FLAGS.train_file, is_training=True)
    ...
    if FLAGS.do_predict:
        eval_examples = read_contract_examples(
